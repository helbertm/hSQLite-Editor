...
export const I18N_MESSAGE_ROWS = Object.freeze([
  ...
  , ["help.intro", "Learn about supported SQLite files, editor tools, and what stays or must be reopened when you update, refresh, or return.", "Conheça os arquivos SQLite compatíveis, as ferramentas do editor e o que permanece ou precisa ser reaberto ao atualizar, recarregar ou voltar.", "Conozca los archivos SQLite compatibles, las herramientas del editor y lo que permanece o debe volver a abrirse al actualizar, recargar o regresar."]
  , ["help.navLabel", "Help topics", "Tópicos da ajuda", "Temas de ayuda"]
  , ["help.startTitle", "Getting started", "Primeiros passos", "Primeros pasos"]
  , ["help.startHtml", "Open a SQLite file, write SQL in a tab, and press <span class=\"kbd\">F5</span>. Use Schema to explore objects and insert names into the editor.", "Abra um arquivo SQLite, escreva SQL em uma aba e pressione <span class=\"kbd\">F5</span>. Use o Schema para explorar objetos e inserir nomes no editor.", "Abra un archivo SQLite, escriba SQL en una pestaña y pulse <span class=\"kbd\">F5</span>. Use Esquema para explorar objetos e insertar nombres en el editor."]
  , ["help.filesTitle", "SQLite files", "Arquivos SQLite", "Archivos SQLite"]
  , ["help.filesExtensions", "Accepted extensions:", "Extensões aceitas:", "Extensiones aceptadas:"]
  , ["help.filesExtensionsLabel", "Accepted SQLite extensions", "Extensões SQLite aceitas", "Extensiones SQLite aceptadas"]
  , ["help.filesFormatHtml", "Besides the extension, the file must be a valid SQLite 3 database with the <code>SQLite format 3</code> signature. Renaming another file to <code>.db</code> does not make it compatible. SQLite 1 and 2 files are not supported.", "Além da extensão, o arquivo precisa ser um banco SQLite 3 válido e conter a assinatura <code>SQLite format 3</code>. Apenas renomear outro arquivo para <code>.db</code> não o torna compatível. Arquivos SQLite 1 e 2 não são abertos.", "Además de la extensión, el archivo debe ser una base de datos SQLite 3 válida y contener la firma <code>SQLite format 3</code>. Cambiar el nombre de otro archivo a <code>.db</code> no lo hace compatible. Los archivos SQLite 1 y 2 no son compatibles."]
  , ["help.workspaceTitle", "Workspace", "Área de trabalho", "Área de trabajo"]
  , ["help.workspaceBody", "Use independent SQL tabs to organize queries. In Schema, filter objects and insert qualified identifiers. In results, sort, paginate, select, and export rows as CSV or JSON.", "Use abas SQL independentes para organizar consultas. No Schema, filtre objetos e insira identificadores qualificados. Nos resultados, ordene, pagine, selecione e exporte registros como CSV ou JSON.", "Use pestañas SQL independientes para organizar consultas. En Esquema, filtre objetos e inserte identificadores cualificados. En los resultados, ordene, pagine, seleccione y exporte filas como CSV o JSON."]
  , ["help.historyTitle", "History and favorites", "Histórico e favoritas", "Historial y favoritas"]
  , ["help.historyBody", "History records recently executed queries, including successful runs and errors. Search or load an entry into the active tab. Successfully executed queries can be saved as favorites for reuse.", "O Histórico registra consultas executadas recentemente, incluindo sucessos e erros. Pesquise ou carregue uma entrada na aba ativa. Consultas executadas com sucesso podem ser salvas como favoritas para reutilização.", "El Historial registra las consultas ejecutadas recientemente, incluidos resultados correctos y errores. Busque o cargue una entrada en la pestaña activa. Las consultas ejecutadas correctamente pueden guardarse como favoritas para reutilizarlas."]
  , ["help.historyLoadNote", "Loading an entry from History or Favorites replaces the current SQL in the active tab. Both are stored only in this browser profile.", "Ao carregar uma entrada do Histórico ou das Favoritas, o SQL atual da aba ativa é substituído. Ambos ficam armazenados somente no perfil atual deste navegador.", "Al cargar una entrada del Historial o de Favoritas, se sustituye el SQL actual de la pestaña activa. Ambos se almacenan únicamente en este perfil del navegador."]
  , ["help.sqlMapTitle", "SQL Map", "Mapa SQL", "Mapa SQL"]
  , ["help.sqlMapBodyHtml", "SQL Map presents the current schema as an interactive entity-relationship diagram. Use it to identify tables, fields, keys, and relationship paths, select related data, and compare the generated <code>SELECT</code> and <code>JOIN</code> statements.", "O Mapa SQL apresenta o schema atual como um diagrama entidade-relacionamento interativo. Use-o para identificar tabelas, campos, chaves e caminhos entre relacionamentos, selecionar dados relacionados e comparar as instruções <code>SELECT</code> e <code>JOIN</code> geradas.", "El Mapa SQL presenta el esquema actual como un diagrama entidad-relación interactivo. Úselo para identificar tablas, campos, claves y rutas entre relaciones, seleccionar datos relacionados y comparar las sentencias <code>SELECT</code> y <code>JOIN</code> generadas."]
  , ["help.sqlMapTeachingTitle", "Teaching use", "Uso didático", "Uso didáctico"]
  , ["help.sqlMapTeachingBody", "In relational database classes, explore the difference between primary and foreign keys, relationship direction, join paths, and the effects of missing or incorrect links. Export the diagram as PNG for discussion or assignments.", "Em aulas de bancos de dados relacionais, explore a diferença entre chave primária e estrangeira, direção dos relacionamentos, caminhos de junção e efeitos de vínculos ausentes ou incorretos. Exporte o diagrama como PNG para discussão ou atividades.", "En clases de bases de datos relacionales, explore la diferencia entre claves primarias y foráneas, la dirección de las relaciones, las rutas de unión y los efectos de vínculos ausentes o incorrectos. Exporte el diagrama como PNG para debates o actividades."]
  , ["help.sqlMapVirtualTitle", "Virtual relationships", "Relacionamentos virtuais", "Relaciones virtuales"]
  , ["help.sqlMapVirtualBody", "Create virtual relationships to test hypotheses without changing the database schema. They appear as dotted lines, undergo compatibility checks, and exist only for the current session.", "Crie relacionamentos virtuais para testar hipóteses sem alterar o schema do banco. Eles aparecem como linhas pontilhadas, passam por verificações de compatibilidade e existem somente na sessão atual.", "Cree relaciones virtuales para probar hipótesis sin cambiar el esquema de la base de datos. Aparecen como líneas de puntos, pasan por comprobaciones de compatibilidad y existen únicamente durante la sesión actual."]
  , ["help.continuityTitle", "Session and updates", "Sessão e atualizações", "Sesión y actualizaciones"]
  , ["help.continuityIntro", "Persisted items stay only in the local storage of this browser profile.", "Os itens persistidos ficam somente no armazenamento local do perfil atual deste navegador.", "Los elementos persistidos permanecen únicamente en el almacenamiento local de este perfil del navegador."]
  , ["help.storedTitle", "Stays in this browser", "Permanece neste navegador", "Permanece en este navegador"]
  , ["help.storedLibrary", "History, favorites, theme, language, preferences, and recent-database metadata.", "Histórico, favoritas, tema, idioma, preferências e metadados de bancos recentes.", "Historial, favoritas, tema, idioma, preferencias y metadatos de bases de datos recientes."]
  , ["help.storedSession", "With Save session enabled: open tabs, tab names, the active tab, and draft SQL.", "Com Salvar sessão ativado: abas abertas, nomes das abas, aba ativa e rascunhos SQL.", "Con Guardar sesión activado: pestañas abiertas, nombres de pestañas, pestaña activa y borradores SQL."]
  , ["help.notRestoredTitle", "Not restored automatically", "Não é restaurado automaticamente", "No se restaura automáticamente"]
  , ["help.notRestoredDatabase", "The database loaded in memory and changes not yet saved to a .db file.", "O banco carregado em memória e alterações ainda não salvas em um arquivo .db.", "La base de datos cargada en memoria y los cambios que aún no se han guardado en un archivo .db."]
  , ["help.notRestoredRuntime", "Results, selections, pagination, and temporary virtual relationships in SQL Map.", "Resultados, seleções, paginação e relacionamentos virtuais temporários do Mapa SQL.", "Resultados, selecciones, paginación y relaciones virtuales temporales del Mapa SQL."]
  , ["help.eventsCaption", "What happens in each situation", "O que acontece em cada situação", "Qué ocurre en cada situación"]
  , ["help.eventColumn", "Situation", "Situação", "Situación"]
  , ["help.keptColumn", "What stays", "O que permanece", "Lo que permanece"]
  , ["help.actionColumn", "What needs attention", "O que exige atenção", "Lo que requiere atención"]
  , ["help.eventRelease", "New version", "Nova versão", "Nueva versión"]
  , ["help.eventReleaseKept", "Local data remains available when you use the same address, browser, and profile.", "Os dados locais continuam disponíveis ao usar o mesmo endereço, navegador e perfil.", "Los datos locales siguen disponibles al usar la misma dirección, navegador y perfil."]
  , ["help.eventReleaseAction", "Save database changes to .db before updating.", "Salve alterações do banco em .db antes de atualizar.", "Guarde los cambios de la base de datos en .db antes de actualizar."]
  , ["help.eventRefresh", "Refresh the page", "Atualizar a página", "Actualizar la página"]
  , ["help.eventRefreshKept", "Local data and, with Save session enabled, tabs and draft SQL.", "Dados locais e, com Salvar sessão ativado, abas e rascunhos SQL.", "Datos locales y, con Guardar sesión activado, pestañas y borradores SQL."]
  , ["help.eventRefreshAction", "Reopen the database; results and temporary state do not return.", "Reabra o banco; resultados e estado temporário não retornam.", "Vuelva a abrir la base de datos; los resultados y el estado temporal no regresan."]
  , ["help.eventReopen", "Close and reopen", "Fechar e abrir novamente", "Cerrar y volver a abrir"]
  , ["help.eventReopenKept", "The same local data; tabs and drafts return when Save session is enabled.", "Os mesmos dados locais; abas e rascunhos retornam quando Salvar sessão está ativado.", "Los mismos datos locales; las pestañas y los borradores regresan cuando Guardar sesión está activado."]
  , ["help.eventReopenAction", "Choose the database again or use Recent; the browser may request permission.", "Escolha o banco novamente ou use Recentes; o navegador pode pedir permissão.", "Elija de nuevo la base de datos o use Recientes; el navegador puede solicitar permiso."]
  , ["help.backupTitle", "Backup and offline mode", "Backup e modo offline", "Copia de seguridad y modo sin conexión"]
  , ["help.backupHtml", "Periodically export a JSON backup from Settings. It may contain SQL, history, favorites, preferences, and tabs, but <strong>does not include the SQLite database</strong>; save the database separately as <code>.db</code>. Clearing site data, using a private window, or changing browser, profile, or device can make local data unavailable.", "Exporte periodicamente um backup JSON em Configurações. Ele pode conter SQL, histórico, favoritas, preferências e abas, mas <strong>não inclui o banco SQLite</strong>; salve o banco separadamente em <code>.db</code>. Limpar dados do site, usar janela privativa ou trocar de navegador, perfil ou dispositivo pode tornar os dados locais indisponíveis.", "Exporte periódicamente una copia JSON desde Configuración. Puede contener SQL, historial, favoritas, preferencias y pestañas, pero <strong>no incluye la base de datos SQLite</strong>; guarde la base por separado como <code>.db</code>. Borrar los datos del sitio, usar una ventana privada o cambiar de navegador, perfil o dispositivo puede hacer que los datos locales no estén disponibles."]
  , ["help.offlineHtml", "The official standalone HTML works from <code>file://</code>. Depending on the browser, changing the HTML name or path may create separate local storage; export a backup before replacing the file.", "O HTML standalone oficial funciona em <code>file://</code>. Dependendo do navegador, mudar o nome ou o caminho do HTML pode criar outro armazenamento local; exporte o backup antes de substituir o arquivo.", "El HTML autónomo oficial funciona desde <code>file://</code>. Según el navegador, cambiar el nombre o la ruta del HTML puede crear otro almacenamiento local; exporte la copia antes de sustituir el archivo."]
  , ["help.privacyTitle", "Privacy and usage statistics", "Privacidade e estatísticas de uso", "Privacidad y estadísticas de uso"]
  , ["help.privacyStandalone", "The downloaded standalone HTML, <code>file://</code> use, and local development do not send usage statistics.", "O HTML standalone baixado, o uso por <code>file://</code> e a execução local não enviam estatísticas de uso.", "El HTML autónomo descargado, el uso mediante <code>file://</code> y la ejecución local no envían estadísticas de uso."]
  , ["help.privacyPages", "Only the official GitHub Pages site uses Umami to measure pageviews, referrers, browser, operating system, device, language, screen size, country, and editor version. URL query parameters and fragments are excluded, and DNT and GPC are respected.", "Somente o site oficial no GitHub Pages usa Umami para medir visualizações, origem de referência, navegador, sistema operacional, dispositivo, idioma, tamanho da tela, país e versão do editor. Parâmetros e fragmentos da URL são excluídos, e DNT e GPC são respeitados.", "Solo el sitio oficial de GitHub Pages usa Umami para medir visitas, referencias, navegador, sistema operativo, dispositivo, idioma, tamaño de pantalla, país y versión del editor. Se excluyen los parámetros y fragmentos de la URL, y se respetan DNT y GPC."]
  , ["help.privacyNoContent", "No database, SQL text, file name, query, history entry, favorite, setting, or tab content is sent for these statistics.", "Nenhum banco, texto SQL, nome de arquivo, consulta, item do histórico, favorita, configuração ou conteúdo de aba é enviado para essas estatísticas.", "No se envía ninguna base de datos, texto SQL, nombre de archivo, consulta, elemento del historial, favorito, configuración ni contenido de pestaña para estas estadísticas."]
  , ["help.privacyNotice", "Read the complete privacy notice", "Ler o aviso de privacidade completo", "Leer el aviso de privacidad completo"]
  , ["help.supportTitle", "Support and suggestions", "Suporte e sugestões", "Soporte y sugerencias"]
  , ["help.supportBody", "Use Report a bug for reproducible defects. Suggest improvement opens the scoped GitHub Feature request form.", "Use Reportar erro para defeitos reproduzíveis. Sugerir melhoria abre o formulário específico Feature request do GitHub.", "Use Informar de un error para defectos reproducibles. Sugerir una mejora abre el formulario específico Feature request de GitHub."]
  ...
  , ["tabs.closeAll", "Close all tabs", "Fechar todas as abas", "Cerrar todas las pestañas"]
  , ["tabs.closeAllTitleGeneric", "Close all tabs?", "Fechar todas as abas?", "¿Cerrar todas las pestañas?"]
  , ["tabs.closeAllTitle", "Close all {count} tabs?", "Fechar todas as {count} abas?", "¿Cerrar las {count} pestañas?"]
  , ["tabs.closeAllWarningGeneric", "All query text and results in these tabs will be lost. A new tab will open.", "Todo o texto SQL e os resultados destas abas serão perdidos. Uma nova aba será aberta.", "Todo el texto SQL y los resultados de estas pestañas se perderán. Se abrirá una nueva pestaña."]
  , ["tabs.closeAllWarning", "All query text and results in these {count} tabs will be lost. A new tab will open.", "Todo o texto SQL e os resultados destas {count} abas serão perdidos. Uma nova aba será aberta.", "Todo el texto SQL y los resultados de estas {count} pestañas se perderán. Se abrirá una nueva pestaña."]
  ...
  , ["footer.privacy", "Privacy", "Privacidade", "Privacidad"]
